    ...
    for(i in 1:n.perm) {
      ...
      for(ii in nchr1) {
        for(jj in nchr2) {
          ...
          temp[[paste(ii,jj,sep="_")]] <- subrousummaryscantwo(tem,for.perm=FALSE)
        }
      }
      tem<-do.call(rbind, temp)

      max2<-function(x) ifelse(length(x)==1, x, x[order(-x)][2])
      out <- c(full = max2(tem$jnt.lod.full),
               fv1 = max2(tem$jnt.lod.full - tem$lod.1qtl),
               int = max2(tem$jnt.lod.full - tem$add.lod.add),
               add = max2(tem$add.lod.add),
               av1 = max2(tem$add.lod.add - tem$lod.1qtl),
               one = max2(tem$lod.1qtl))


      # maxima
      for(j in 1:6) perm.result[[j]][i,] <- out[[j]]
    }
