...
#'
#' @param mod A QTL model containing LOD profiles. Can be generated by stepwiseqtl (with
#' keepLodProfile = TRUE), or by refineqtl.
...
calcCis<-function(mod, qtlnames = NULL, lodint = TRUE, drop=1.5, prob=0.95, ...){
  if(is.null(attr(mod, "lodprofile"))) stop("LOD profiles must be included in the QTL model\n")
  ...
  out<-lapply(1:nqtl(mod), function(j){
    ...
    chr=mod$chr[j]
    pos=mod$pos[j]
    return(data.frame(qtlname = qtlnames[j], chr, pos, maxLod, lowmarker,highmarker,lowposition,highposition))
  })
